    ...
    : AudioProcessorEditor (p), processor (p)
{
    ...
    for (int i = 0; i < stageNames.size(); ++i)
    {
        ...
        btn->setColour (juce::TextButton::buttonOnColourId, juce::Colour (0xFFE04565));
    ...
    auto addKnob = [&] (const juce::String& paramID, const juce::String& label, int stage) -> juce::Slider*
    {
        ...
        s->setColour (juce::Slider::rotarySliderFillColourId, juce::Colour (0xFFE9A045));
        s->setVisible (false);
        addAndMakeVisible (s);
        allSliders.add (s);
    ...
    masterOutputSlider.setColour (juce::Slider::textBoxTextColourId, juce::Colour (0xFFCCCCEE));
    ...
    // ─── Inline crossover frequency editor ────────────
    addChildComponent (xoverFreqEditor);
    xoverFreqEditor.setJustification (juce::Justification::centred);
    xoverFreqEditor.setFont (juce::Font (12.0f, juce::Font::bold));
    xoverFreqEditor.setColour (juce::TextEditor::backgroundColourId, juce::Colour (0xFF1A1A3A));
    xoverFreqEditor.setColour (juce::TextEditor::textColourId, juce::Colours::white);
    xoverFreqEditor.setColour (juce::TextEditor::outlineColourId, juce::Colour (0xFFE9A045));
    xoverFreqEditor.onReturnKey = [this] {
        float freq = xoverFreqEditor.getText().getFloatValue();
        if (freq >= 20.0f && freq <= 20000.0f && xoverEditParamID.isNotEmpty())
        {
            auto* param = processor.getAPVTS().getParameter (xoverEditParamID);
            if (param)
            {
        ...
        xoverFreqEditor.setVisible (false);
        xoverEditParamID = "";
        repaint();
    };
    xoverFreqEditor.onFocusLost = [this] {
        xoverFreqEditor.setVisible (false);
        xoverEditParamID = "";
    };
...
void EasyMasterEditor::paint (juce::Graphics& g)
{
    ...
    if (tabButtons.size() > 0)
    {
        ...
        if (currentStage == 0)
        {
            ...
            float dispY = meterArea.getY() + 145.0f; // below knobs + text values
            float totalH = meterArea.getHeight() - 155.0f;
        ...
        if (currentStage == 2)
        {
            auto* comp = dynamic_cast<CompressorStage*> (
            ...
            if (comp)
            {
                ...
                {
                    float yy = plotY + plotH * (-db / dbScale);
                    g.drawHorizontalLine ((int) yy, plotX, plotX + plotW);
                }
                // 0 dB reference
                g.setColour (juce::Colour (0xFF333340));
                g.drawHorizontalLine ((int) plotY, plotX, plotX + plotW);
                // dB labels
                g.setColour (juce::Colour (0xFF555566));
                g.setFont (juce::Font (8.0f));
                for (float db = -25.0f; db <= 0.0f; db += 5.0f)
                {
                    float yy = plotY + plotH * (-db / dbScale);
                    g.drawText (juce::String ((int) db), (int) dispX, (int)(yy - 5), 30, 10, juce::Justification::centredRight);
                }

                // Draw input waveform (light gray, scrolling)
                auto& inputHist = comp->getInputHistory();
                auto& grHist = comp->getGRHistory();
                int histPos = comp->getGRHistoryPos();
                int histSize = CompressorStage::GR_HISTORY_SIZE;
                ...
                // Input level waveform
                {
                    ...
                    for (int i = 0; i < histSize; ++i)
                    {
                        ...
                        float x = plotX + (float) i / (float) histSize * plotW;
                        float db = inputHist[(size_t) idx];
                        float y = plotY + plotH * juce::jlimit (0.0f, 1.0f, -db / dbScale);
                        if (!started) { inputPath.startNewSubPath (x, y); started = true; }
                        else inputPath.lineTo (x, y);
                    }
                    if (started)
                    {
                        juce::Path fillP (inputPath);
                        fillP.lineTo (plotX + plotW, plotY + plotH);
                        fillP.lineTo (plotX, plotY + plotH);
                        fillP.closeSubPath();
                        g.setColour (juce::Colour (0xFFAABBCC).withAlpha (0.06f));
                        g.fillPath (fillP);
                        g.setColour (juce::Colour (0xFFAABBCC).withAlpha (0.25f));
                        g.strokePath (inputPath, juce::PathStrokeType (0.8f));
                    }
                }

                // GR curve (red/orange, prominent)
                {
                    juce::Path grPath;
                    bool started = false;
                    for (int i = 0; i < histSize; ++i)
                    {
                        int idx = (histPos + i) % histSize;
                        float x = plotX + (float) i / (float) histSize * plotW;
                        float grDb = grHist[(size_t) idx];
                        float y = plotY + plotH * juce::jlimit (0.0f, 1.0f, -grDb / dbScale);
                    ...
                    {
                        // Filled GR area
                        juce::Path fillP (grPath);
                        fillP.lineTo (plotX + plotW, plotY);
                        fillP.lineTo (plotX, plotY);
                        fillP.closeSubPath();
                        g.setColour (juce::Colour (0xFFFF6655).withAlpha (0.12f));
                        g.fillPath (fillP);
                        // GR stroke
                        g.setColour (juce::Colour (0xFFFF5544).withAlpha (0.9f));
                        g.strokePath (grPath, juce::PathStrokeType (1.5f));
                    }
                }

                // Threshold line
                float thresh = comp->getMeterData().gainReduction.load(); // current GR
                float threshDb = processor.getAPVTS().getRawParameterValue ("S3_Comp_Threshold")->load();
                float threshY = plotY + plotH * juce::jlimit (0.0f, 1.0f, -threshDb / dbScale);
                g.setColour (juce::Colour (0xFFE9A045).withAlpha (0.6f));
                float dashLen = 6.0f;
                for (float dx = plotX; dx < plotX + plotW; dx += dashLen * 2)
                    g.fillRect (dx, threshY - 0.5f, dashLen, 1.0f);

                // Current GR value readout
                float gr = comp->getMeterData().gainReduction.load();
                g.setColour (juce::Colours::white);
                g.setFont (juce::Font (12.0f, juce::Font::bold));
                g.drawText (juce::String (gr, 1) + " dB", dispX + dispW - 80.0f, dispY + 2.0f, 70.0f, 14.0f, juce::Justification::centredRight);

                // Bottom meter bar (compact)
                auto barArea = juce::Rectangle<float> (plotX, plotY + plotH + 6.0f, plotW, 14.0f);
                ...
                if (normalized > 0.001f)
                {
                    ...
                    juce::ColourGradient grGrad (juce::Colour (0xFFDD4466), barArea.getRight() - fillW, barArea.getY(),
                                                 juce::Colour (0xFFFF6655), barArea.getRight(), barArea.getY(), false);
                ...
                for (int db = 0; db >= -20; db -= 5)
                {
                    ...
                    g.drawText (juce::String (db), (int)xPos - 10, (int)barArea.getBottom() + 1, 20, 9, juce::Justification::centred);
                }
            }
        }
        ...
        if (currentStage == 8)
        {
            ...
            g.drawHorizontalLine ((int) dbToY (0), specX, specX + specW);

            // dB labels (overlaid on spectrum)
            g.setColour (juce::Colour (0xFF4A4A6A).withAlpha (0.7f));
            g.setFont (juce::Font (8.0f));
            for (float db = -50.0f; db <= 0.0f; db += 10.0f)
                g.drawText (juce::String ((int) db), (int) specX + 2, (int) dbToY (db) - 5, 26, 10, juce::Justification::centredLeft);
            ...
            g.setFont (juce::Font (8.0f));
            float labelFreqs[] = { 50, 100, 200, 500, 1000, 2000, 5000, 10000 };
            for (auto f : labelFreqs)
            {
                float xPos = freqToX (f);
                juce::String label = (f >= 1000) ? juce::String (f / 1000.0f, 0) + "k" : juce::String ((int) f);
                g.drawText (label, (int)(xPos - 16), (int)(specY + specH + 2), 32, 10, juce::Justification::centred);
            }
            ...
            juce::Colour bandCols[] = { juce::Colour (0xFF5599DD), juce::Colour (0xFF55CC99), juce::Colour (0xFFDDAA44), juce::Colour (0xFFDD5555) };
            ...
            // ─── Band regions with gain reduction fill (Pro-MB style) ───
            for (int b = 0; b < 4; ++b)
            {
                ...
                // Band fill — gradient from top (colored) to threshold (transparent)
                float thresh = processor.getAPVTS().getRawParameterValue (
                    "S8_MBDyn_B" + juce::String (b + 1) + "_Thresh")->load();
                float threshY = dbToY (thresh);
                threshY = juce::jlimit (specY, specY + specH, threshY);

                juce::ColourGradient bandGrad (bandCols[b].withAlpha (sel ? 0.20f : 0.10f), bx0, specY,
                                               bandCols[b].withAlpha (0.02f), bx0, threshY, false);
                g.setGradientFill (bandGrad);
                g.fillRect (bx0, specY, bw, threshY - specY);

                // Below threshold — subtle fill
                g.setColour (bandCols[b].withAlpha (sel ? 0.06f : 0.02f));
                g.fillRect (bx0, threshY, bw, specY + specH - threshY);

                // GR fill — gradient fade showing gain reduction amount
                if (mbStage)
                {
                    float gr = mbStage->bandGRDisplay[(size_t) b].load (std::memory_order_relaxed);
                    if (std::abs (gr) > 0.1f)
                    {
                        float grDb = std::abs (gr);
                        float grH = juce::jlimit (2.0f, specH * 0.5f, grDb * specH / dbRange);
                        if (gr < 0) // compression: fill below threshold
                        {
                            juce::ColourGradient grGrad (bandCols[b].withAlpha (0.35f), bx0, threshY,
                                                         bandCols[b].withAlpha (0.05f), bx0, threshY + grH, false);
                            g.setGradientFill (grGrad);
                            g.fillRect (bx0 + 1, threshY, bw - 2, grH);
                        }
                        else // expansion: fill above threshold
                        {
                            juce::ColourGradient grGrad (bandCols[b].withAlpha (0.35f), bx0, threshY,
                                                         bandCols[b].withAlpha (0.05f), bx0, threshY - grH, false);
                            g.setGradientFill (grGrad);
                            g.fillRect (bx0 + 1, threshY - grH, bw - 2, grH);
                        }
                        // GR value text
                        g.setColour (bandCols[b].withAlpha (0.8f));
                        g.setFont (juce::Font (8.0f));
                        juce::String grLabel = juce::String (gr, 1) + " dB";
                        float labelY = (gr < 0) ? threshY + grH + 2 : threshY - grH - 12;
                        g.drawText (grLabel, (int)((bx0 + bx0 + bw) * 0.5f - 20), (int) labelY, 40, 10, juce::Justification::centred);
                    }
                }

                // Threshold line — solid, colored (Pro-MB style)
                g.setColour (bandCols[b].withAlpha (0.9f));
                g.fillRect (bx0 + 2, threshY - 0.5f, bw - 4, 1.5f);

                // Threshold handle dot (center of band)
                float dotX = (bx0 + bx1) * 0.5f;
                g.setColour (bandCols[b]);
                g.fillEllipse (dotX - 5.0f, threshY - 5.0f, 10.0f, 10.0f);
                g.setColour (juce::Colour (0xFF000000).withAlpha (0.3f));
                g.drawEllipse (dotX - 5.0f, threshY - 5.0f, 10.0f, 10.0f, 0.5f);

                // Threshold dB label (small, next to dot)
                g.setColour (bandCols[b]);
                g.setFont (juce::Font (9.0f, juce::Font::bold));
                g.drawText (juce::String ((int) thresh) + " dB", (int)(dotX + 8), (int)(threshY - 6), 44, 12, juce::Justification::centredLeft);

                // Band name at top
                g.setColour (bandCols[b].withAlpha (0.8f));
                g.setFont (juce::Font (9.0f, juce::Font::bold));
                g.drawText (bandNames[b], (int)((bx0 + bx1) * 0.5f - 22), (int) specY + 4, 44, 11, juce::Justification::centred);

                // Solo button (S) — bottom left of each band
                float soloW = juce::jmin (bw * 0.18f, 22.0f), soloH = 14.0f;
                float soloX = bx0 + 4.0f, soloY2 = specY + specH - 18.0f;
                mbDynSoloBtnRects[(size_t) b] = { soloX, soloY2, soloW, soloH };
                bool isSoloed = false;
                auto* sp = processor.getAPVTS().getRawParameterValue ("S8_MBDyn_B" + juce::String (b + 1) + "_Solo");
                if (sp) isSoloed = sp->load() > 0.5f;
                g.setColour (isSoloed ? juce::Colour (0xFFFFCC00) : juce::Colour (0xFF222240));
                g.fillRoundedRectangle (soloX, soloY2, soloW, soloH, 3.0f);
                if (!isSoloed) { g.setColour (juce::Colour (0xFF333355)); g.drawRoundedRectangle (soloX, soloY2, soloW, soloH, 3.0f, 0.5f); }
                g.setColour (isSoloed ? juce::Colour (0xFF000000) : juce::Colour (0xFF888888));
            ...
            for (int c = 0; c < 3; ++c)
            {
                ...
                // Subtle glow
                g.setColour (juce::Colour (0xFFCCCCEE).withAlpha (0.08f));
                g.fillRect (xPos - 3.0f, specY, 6.0f, specH);
                // Main line
                g.setColour (juce::Colour (0xFFAABBDD).withAlpha (0.6f));
                g.drawVerticalLine ((int) xPos, specY, specY + specH);
                // Top triangle handle
                juce::Path triTop;
                triTop.addTriangle (xPos - 5, specY, xPos + 5, specY, xPos, specY + 8);
                ...
                g.setFont (juce::Font (9.0f, juce::Font::bold));
                juce::String fLabel = (xoverFreqs[c] >= 1000) ? juce::String (xoverFreqs[c] / 1000.0f, 1) + "k" : juce::String ((int) xoverFreqs[c]);
                g.drawText (fLabel, (int)(xPos - 20), (int)(specY + specH + 12), 40, 12, juce::Justification::centred);
            }

            // ─── POPUP PANEL (Pro-MB floating panel style) ───
            if (mbDynSelectedBand >= 0 && mbDynSelectedBand < 4)
            {
                ...
                float popW = 600.0f, popH = 150.0f;
                float popX = juce::jlimit (dispX + 4, dispX + dispW - popW - 4, (bx0 + bx1) * 0.5f - popW * 0.5f);
                float popY = juce::jmax (dispY - popH - 10.0f, panelArea.getY() + 30.0f);

                // Shadow
                g.setColour (juce::Colour (0x44000000));
                g.fillRoundedRectangle (popX + 2, popY + 2, popW, popH, 8.0f);
                // Panel background
                g.setColour (juce::Colour (0xF0101028));
                g.fillRoundedRectangle (popX, popY, popW, popH, 8.0f);
                // Colored top accent line
                g.setColour (bandCols[sb]);
                g.fillRoundedRectangle (popX + 2, popY, popW - 4, 3.0f, 1.5f);
                // Border
                g.setColour (juce::Colour (0xFF333355));
                g.drawRoundedRectangle (popX, popY, popW, popH, 8.0f, 0.8f);

                // Band name
                g.setColour (bandCols[sb]);
                g.setFont (juce::Font (11.0f, juce::Font::bold));
                g.drawText (bandNames[sb], (int)(popX + 10), (int)(popY + 6), 80, 14, juce::Justification::centredLeft);

                // Close button
                g.setColour (juce::Colour (0xFF666688));
                g.setFont (juce::Font (14.0f));
                g.drawText ("X", (int)(popX + popW - 22), (int)(popY + 4), 18, 16, juce::Justification::centred);

                // Bypass toggle
                bool isBypassed = false;
                auto* byp = processor.getAPVTS().getRawParameterValue ("S8_MBDyn_B" + juce::String (sb + 1) + "_Bypass");
                if (byp) isBypassed = byp->load() > 0.5f;
                float bypX = popX + 10, bypY = popY + 50;
                g.setColour (isBypassed ? juce::Colour (0xFFFF6644) : juce::Colour (0xFF333355));
                g.fillRoundedRectangle (bypX, bypY, 40, 18, 3.0f);
                g.setColour (isBypassed ? juce::Colours::white : juce::Colour (0xFF888888));
                g.setFont (juce::Font (9.0f, juce::Font::bold));
                g.drawText ("BYP", (int) bypX, (int) bypY, 40, 18, juce::Justification::centred);
            }
        }
...
{
    // ─── Top bar: Channel + XMode combos (small, above spectrum) ───
    auto topRow = panelArea.removeFromTop (26);
    int gCol = 0;
    for (int i = 0; i < allCombos.size(); ++i)
    {
    ...
    // ─── Hide ALL per-band controls by default ───
    for (int i = 0; i < allCombos.size(); ++i)
        if (comboStage[i] == kMBDynBand) { allCombos[i]->setVisible (false); comboLabels[i]->setVisible (false); }
    for (int i = 0; i < allSliders.size(); ++i)
        if (stageForControl[i] == kMBDynBand) { allSliders[i]->setVisible (false); allLabels[i]->setVisible (false); }

    // ─── Show selected band's controls in popup position ───
    if (mbDynSelectedBand < 0 || mbDynSelectedBand > 3) return;
    ...
    // Compute popup position from window bounds (same as paint's formula)
    auto origPanel = getLocalBounds().withTop (95).withBottom (getHeight() - 70).reduced (8).toFloat();
    auto mba = origPanel.reduced (12.0f);
    float mY = mba.getBottom() - 50.0f;
    float mX = mba.getX();
    float mW = mba.getWidth();
    float specDispY = mY - 300.0f;
    ...
    float xf3 = processor.getAPVTS().getRawParameterValue ("S8_MBDyn_Xover3")->load();
    float bandEdges[] = { 20.0f, xf1, xf2, xf3, 20000.0f };
    float x0 = freqToXl (bandEdges[sb]);
    float x1 = freqToXl (bandEdges[sb + 1]);
    float popW = 600.0f, popH = 150.0f;
    float popX = juce::jlimit (mX + 4, mX + mW - popW - 4, (x0 + x1) * 0.5f - popW * 0.5f);
    float popY = specDispY - popH - 10.0f;

    // Position per-band combo (Mode) in popup
    struct BandCtrl { int comboIdx = -1; int knobs[8] = {-1,-1,-1,-1,-1,-1,-1,-1}; };
    std::array<BandCtrl, 4> bc;
    int bcc = 0;
    for (int i = 0; i < allCombos.size(); ++i)
        if (comboStage[i] == kMBDynBand && bcc < 4) { bc[(size_t)bcc].comboIdx = i; bcc++; }
    int bkc = 0;
    for (int i = 0; i < allSliders.size(); ++i)
    {
        if (stageForControl[i] != kMBDynBand) continue;
        int bnd = bkc / 8, kid = bkc % 8;
        if (bnd < 4) bc[(size_t)bnd].knobs[kid] = i;
        bkc++;
    }

    // Mode combo on the left
    if (bc[sb].comboIdx >= 0)
    {
        int ci = bc[sb].comboIdx;
        allCombos[ci]->setBounds ((int)(popX + 10), (int)(popY + 22), 110, 24);
        allCombos[ci]->setVisible (true);
        comboLabels[ci]->setVisible (false);
    }

    // 8 knobs in a clean row (Thresh, Range, Ratio, Atk, Rel, Knee, Out, Mix)
    int knobSz = 38;
    float knobAreaX = popX + 120;
    float knobAreaW = popW - 130;
    int kSpacing = (int)(knobAreaW / 8);
    for (int k = 0; k < 8; ++k)
    {
        int ki = bc[sb].knobs[k];
        if (ki < 0) continue;
        int px = (int)(knobAreaX + k * kSpacing + (kSpacing - knobSz) / 2);
        int py = (int)(popY + 22);
        allSliders[ki]->setBounds (px, py, knobSz, knobSz + 13);
        allSliders[ki]->setVisible (true);
        allSliders[ki]->setColour (juce::Slider::rotarySliderFillColourId, bandCols[sb]);
        allLabels[ki]->setBounds (px - 6, py + knobSz + 11, knobSz + 12, 10);
        allLabels[ki]->setVisible (true);
        allLabels[ki]->setFont (juce::Font (8.0f));
    }
}
...
void EasyMasterEditor::mouseDown (const juce::MouseEvent& e)
{
    ...
    if (currentStage == 8 && mbDynDisplayArea.getWidth() > 0)
    {
        ...
        if (mbDynSelectedBand >= 0 && mbDynSelectedBand < 4)
        {
            ...
            // Bypass toggle click
            float bypX = popX + 10, bypY = popY + 50;
            if (pos.x >= bypX && pos.x <= bypX + 40 && pos.y >= bypY && pos.y <= bypY + 18)
            {
                auto* param = processor.getAPVTS().getParameter ("S8_MBDyn_B" + juce::String (mbDynSelectedBand + 1) + "_Bypass");
                if (param)
                {
        ...
        if (mbDynDisplayArea.contains (pos))
        {
            ...
            for (int b = 0; b < 4; ++b)
            {
                ...
                if (pos.x >= bx0 && pos.x <= bx1)
                {
                    ...
                    // Click on band region → select this band (show popup)
                    mbDynSelectedBand = (mbDynSelectedBand == b) ? -1 : b;
                    resized();
                    repaint();
                    return;
                }
...
void EasyMasterEditor::mouseDoubleClick (const juce::MouseEvent& e)
{
    ...
    // ─── SAT crossover double-click (stage 3 multiband) ───
    if (currentStage == kSatCommon && fftDisplayArea.getWidth() > 0)
    {
        float specX = fftDisplayArea.getX() + 8.0f;
        float specW = fftDisplayArea.getWidth() - 16.0f;
        juce::String xoverIDs[] = { "S4_Sat_Xover1", "S4_Sat_Xover2", "S4_Sat_Xover3" };
        for (int c = 0; c < 3; ++c)
        {
            float xf = processor.getAPVTS().getRawParameterValue (xoverIDs[c])->load();
            float xPos = specX + specW * (std::log10 (xf / 20.0f) / std::log10 (20000.0f / 20.0f));
            if (std::abs (pos.x - xPos) < 14.0f)
            {
                xoverEditParamID = xoverIDs[c];
                xoverFreqEditor.setText (juce::String ((int) xf), false);
                xoverFreqEditor.setBounds ((int)(xPos - 35), (int)(pos.y - 12), 70, 24);
    ...
    if (currentStage == 8 && mbDynDisplayArea.getWidth() > 0)
    {
        float specX = mbDynDisplayArea.getX();
        float specW = mbDynDisplayArea.getWidth();
        juce::String xoverIDs[] = { "S8_MBDyn_Xover1", "S8_MBDyn_Xover2", "S8_MBDyn_Xover3" };
        for (int c = 0; c < 3; ++c)
        {
            float xf = processor.getAPVTS().getRawParameterValue (xoverIDs[c])->load();
            float xPos = specX + specW * (std::log10 (xf / 20.0f) / std::log10 (20000.0f / 20.0f));
