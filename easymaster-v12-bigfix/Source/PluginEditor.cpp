...
void EasyMasterEditor::showStage (int tabIndex)
{
    ...
    auto shouldShow = [&](int controlStage) -> bool
    {
        ...
        // Output EQ M/S: show Mid knobs (stage 4) OR Side knobs (kEQSide), never both
        if (stageType == 4 && controlStage == kEQSide)
        {
            int eqMs = (int) processor.getAPVTS().getRawParameterValue ("S5_EQ2_MS")->load();
            return (eqMs > 0 && eqEditSide); // only show Side knobs when editing Side
        }
        // Hide Mid knobs when editing Side
        if (stageType == 4 && controlStage == 4)
        {
            int eqMs = (int) processor.getAPVTS().getRawParameterValue ("S5_EQ2_MS")->load();
            if (eqMs > 0 && eqEditSide) return false; // hide Mid when editing Side
        }
        return false;
    };
...
void EasyMasterEditor::resized()
{
    ...
    auto isVisible = [&](int controlStage) -> bool
    {
        ...
        // Output EQ M/S: show only one set at a time
        if (currentStage == 4)
        {
            int eqMs = (int) processor.getAPVTS().getRawParameterValue ("S5_EQ2_MS")->load();
            if (eqMs > 0)
            {
                if (eqEditSide && controlStage == kEQSide) return true;
                if (!eqEditSide && controlStage == 4) return true;
